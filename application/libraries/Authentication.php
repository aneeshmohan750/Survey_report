...
// ------------------------------------------------------------------------
	...
	class Authentication
	{
		...
		function process_login ($login = NULL, $request = '')
		{
				...
				$this->CI->db->select ('username   AS USERNAME, password, CONCAT_WS(" ", first_name, last_name) as NAME, user_id as USERID, email as EMAIL,is_admin,company_id', false);
				...
				if (0 < $select_query->num_rows())
				{
					  ...
					  $this->CI->db->select('survey_id,survey_type_id');
					  $this->CI->db->where('user_id',$row->USERID);
					  $select_query    = $this->CI->db->get ('survey_user_map');
					  if (0 < $select_query->num_rows())
				      {
						$row 			= $select_query->row();    
					    $session_data['survey_id'] = $row->survey_id;
						$session_data['survey_type_id'] = $row->survey_type_id;
					  }
